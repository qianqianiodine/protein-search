  ...
  useEffect(() => {
    if (prevPhaseRef.current === 'loading_pdb' && phase === 'results' && selectedProtein && structures.length > 0) {
      saveHistory({
        ...
        protein: { accession: selectedProtein.accession, name: selectedProtein.name, gene: selectedProtein.gene, aliases: selectedProtein.aliases, organism: selectedProtein.organism, length: selectedProtein.length },
